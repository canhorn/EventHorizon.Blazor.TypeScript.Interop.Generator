namespace EventHorizon.Blazor.TypeScript.Interop.Generator.AstParser.NodeImpl.Model
{
        ...
        : Node
    {
        ...
        )
        {
            ...
            {
                var bodyArrayChildren = node.BodyArray.Select(
            ...
            Kind = overrideKind ?? NodeJSTypeMapper.NodeJSTypeToSyntaxKind(
                node.Type,
                node.Kind
            );
            ...
            if (node.Implements is not null)
            {
                ...
                {
                    var interfaceNode = new NodeJS_Node(
                        implementation,
                        parent: this
                    );
                    heritageClauses.Add(
            ...
            {
                var types = node.TypeParameters.Params.Select(
    ...
    {
        public static string NodeJSTypeToSyntaxKind(
            string type,
            string kind
        )
        {
            if (type == "TSDeclareMethod"
