namespace EventHorizon.Maui.BabylonJS.Pages
{
    ...
    public partial class Index : IAsyncDisposable
    {
        ...
        {
            if (_firstRenderComplete && _engine is not null)
            {
                try
                {
        ...
        public async Task CreateScene()
        {
            ...
            var light0 = await PointLight.NewPointLight(
                "Omni",
                await Vector3.NewVector3(0, 2, 8),
                scene
            );
            var box1 = await Mesh.CreateBox("b1", 1.0m, scene);
            var box2 = await Mesh.CreateBox("b1", 1.0m, scene);
            await box2.set_position(await Vector3.NewVector3(2m, 0, 0));
            ...
            await Canvas.RemoveBlur("game-window");
        }
    }
}
