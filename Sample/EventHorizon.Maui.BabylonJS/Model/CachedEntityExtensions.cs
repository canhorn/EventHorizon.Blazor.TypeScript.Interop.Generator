namespace EventHorizon.Blazor.Interop
{
    ...
    {
        public static T ToEntity<T>(this CachedEntity entity)
...
namespace BABYLON
{
    ...
    {
        public static async ValueTask setParent(this Node node, string elementId)
        {
        ...
        public static async Task<T> clone<T>(this Node node, string name)
            where T : CachedEntity, new()
        {
            return await EventHorizonBlazorInterop.FuncClass<T>(
                entity => new T() { ___guid = entity.___guid },
                new object[] { new string[] { node.___guid, "clone" }, name }
            );
        }
    }
}
