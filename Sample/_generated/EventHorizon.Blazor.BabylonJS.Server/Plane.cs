...
public class Plane : CachedEntityObject
{
    ...
    public static async ValueTask<Plane> FromArray(decimal[] array)
    {
        return await EventHorizonBlazorInterop.FuncClass<Plane>(
            ...
            new object[] { new string[] { "Plane", "FromArray" }, array }
        );
    }
    ...
    public static async ValueTask<Plane> FromPoints(Vector3 point1, Vector3 point2, Vector3 point3)
    {
        return await EventHorizonBlazorInterop.FuncClass<Plane>(
            entity => new Plane() { ___guid = entity.___guid },
            new object[] { new string[] { "Plane", "FromPoints" }, point1, point2, point3 }
        );
    }
    ...
    public static async ValueTask<Plane> FromPositionAndNormal(Vector3 origin, Vector3 normal)
    {
        return await EventHorizonBlazorInterop.FuncClass<Plane>(
            entity => new Plane() { ___guid = entity.___guid },
            new object[] { new string[] { "Plane", "FromPositionAndNormal" }, origin, normal }
        );
    }

    public static async ValueTask<decimal> SignedDistanceToPlaneFromPositionAndNormal(
        Vector3 origin,
        Vector3 normal,
        Vector3 point
    )
    {
        return await EventHorizonBlazorInterop.Func<decimal>(
            new object[]
            {
                new string[] { "Plane", "SignedDistanceToPlaneFromPositionAndNormal" },
                origin,
                normal,
                point
            }
    ...
    public static async ValueTask<Plane> NewPlane(decimal a, decimal b, decimal c, decimal d)
    {
        var entity = await EventHorizonBlazorInterop.New(new string[] { "Plane" }, a, b, c, d);
