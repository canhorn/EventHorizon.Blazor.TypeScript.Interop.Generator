...
namespace BABYLON
{
    ...
    public class PointerInfoBase : CachedEntityObject
    {
        ...
        public async ValueTask<IMouseEventCachedEntity> get_event()
        {
            if (__event == null)
            {
                __event = await EventHorizonBlazorInterop.GetClass<IMouseEventCachedEntity>(
                    this.___guid,
                    "event",
                    (entity) =>
                    {
        ...
        public ValueTask set_event(IMouseEventCachedEntity value)
        {
            __event = null;
