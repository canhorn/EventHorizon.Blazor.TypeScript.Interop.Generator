...
public partial class PirateFort : IAsyncDisposable
{
    ...
    public async Task CreateSceneAsync()
    {
        ...
        var cannonBall = await MeshBuilder.CreateSphere(
            "cannonBall",
            new { diameter = 0.3 },
            scene
        );
        var cannonBallMaterial = await StandardMaterial.NewStandardMaterial(
            "cannonBallMaterial",
            scene
        );
        ...
        var cannonImportMeshes = await cannonImportResult.get_meshes();
        var cannon = (await cannonImportMeshes[0].getChildren<Node>())[0];
        await cannon.setParent(null);
        await cannonImportMeshes[0].dispose();

        //set the metadata of each mesh to filter on later
        var cannonMeshes = await cannon.getChildMeshes<Node>();
        ...
        var cannonAnimGroup = await AnimationGroup.NewAnimationGroup("cannonAnimGroup", scene);
        await cannonAnimGroup.addTargetedAnimation(
            animations[0],
            (await cannon.getChildMeshes<Node>())[1]
        );
        await cannonAnimGroup.addTargetedAnimation(
            animations[1],
            (await cannon.getChildMeshes<Node>())[0]
        );
        ...
        await particleEmitter.setParent((await cannon.getChildMeshes<Node>())[1]);
        ...
        var cannonPositionArray = new Vector3[][]
        {
            ...
            [
                ...
                await Vector3.NewVector3(
                    await Tools.ToRadians(0),
                    await Tools.ToRadians(90),
                    await BABYLON.Tools.ToRadians(180)
                ),
            ],
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            await cannonAnimGroupClone.addTargetedAnimation(
                ...
                (await cannonClone.getChildMeshes<Node>())[1]
            );
            await cannonAnimGroupClone.addTargetedAnimation(
                await (await cannonAnimGroup.get_targetedAnimations())[1].get_animation(),
                (await cannonClone.getChildMeshes<Node>())[0]
            );
            ...
            async (pointerInfo, eventState) =>
            {
                ...
                if (pickedMesh != null && await pickedMesh.get_metadata() != null)
                {
                    ...
                    if (cannonReadyToPlay[name] == 1)
                    {
                        ...
                        var sceneAnimationGroups = await scene.get_animationGroups();
                        for (var i = 0; i < sceneAnimationGroups.Length; i++)
                        {
                            if (await sceneAnimationGroups[i].get_name() == animationToPlay)
                            {
                                await sceneAnimationGroups[i].play();
                                //after the animation has finished, set the readyToPlay status for this cannon to 1;
                                await (
                                    await sceneAnimationGroups[i]
